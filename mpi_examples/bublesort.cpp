    ...
    for (int i = 0; i < proc_number; i++) {
        if (i % 2 == 1) {
            if (odd_rank >= 0 && odd_rank < proc_number) {
                MPI_Sendrecv(proc_data, proc_data_size, MPI_LONG, odd_rank, 0, received, proc_data_size, MPI_LONG,
                             odd_rank, 0, MPI_COMM_WORLD, &status);
                if (rank % 2 == 1) {
            ...
            if (even_rank >= 0 && even_rank < proc_number) {
                MPI_Sendrecv(proc_data, proc_data_size, MPI_LONG, even_rank, 0, received, proc_data_size, MPI_LONG,
                             even_rank, 0, MPI_COMM_WORLD, &status);
                if (rank % 2 == 0) {
                    compare_exchange_min(proc_data, received, proc_data_size);
                } else {
                    compare_exchange_max(proc_data, received, proc_data_size);
